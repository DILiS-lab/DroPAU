"""
Code adapted from MiVOLO https://github.com/WildChlamydia/MiVOLO
    ...
    def load_from_ckpt(
        ...
        if "with_persons_model" in state:
            ...
            self.with_persons_model = (
                "patch_embed.conv1.0.weight" in state["state_dict"]
            )

        self.num_classes = 2 if only_age else 4
...
class MiVOLO:
    ...
    def fill_in_results(self, output, detected_bboxes, faces_inds, bodies_inds):
        ...
        for index in range(output.shape[0]):
            ...
            age_variance_select = age_variance[index].item()
            age_std = np.sqrt(age_variance_select) * (
                self.meta.max_age - self.meta.min_age
            )
            ...
            _logger.info(f"\tage std: {age_std}")
