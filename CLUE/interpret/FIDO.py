...
class mask_explainer(BaseNet):
    ...
    def fit_cat(self, x, BNN, VAEAC, flatten_ims=True, test_dims=None, plot=False):
        ...
        for it in range(self.mask_samples):
            ...
            aleatoric_cum = aleatoric_cum + aleatoric_entropy.mean().item()
            epistemic_cum = epistemic_cum + epistemic_entropy.mean().item()
            ...
            loss_cum = loss_cum + loss.item() / aleatoric_entropy.shape[0]
    ...
    def fit_gauss(self, x, BNN, VAEAC, flatten_ims=True, test_dims=None, plot=False):
        ...
        for it in range(self.mask_samples):
            ...
            aleatoric_cum = aleatoric_cum + aleatoric_std.mean().item()
            epistemic_cum = epistemic_cum + epistemic_std.mean().item()
            loss = (
            ...
            loss.backward()  # Gradient accumulation
            loss_cum = loss_cum + loss.item() / aleatoric_std.shape[0]
